...
//	objfile << "mtllib " + filename + ".mtl" << "\n" << "\n";
  ...
  for (int i = 0; i < mesh_ptr->vertices().size(); ++i){
    float w = mesh_ptr->vertices()[i][0];
    float x = mesh_ptr->vertices()[i][1];
    float y = mesh_ptr->vertices()[i][2];
    float z = mesh_ptr->vertices()[i][3];
    objfile << "v " << w << " " << x << " " << y << " " << z << "\n";
  }
  std::cout << " finished vertices" << std::endl;
  // Print faces section. Starts with 'f' and separated by spaces.
  for (int i = 0; i < mesh_ptr->indices().size(); i+=4){
  	int one = mesh_ptr->indices()[i] + 1;
  	int two = mesh_ptr->indices()[i+1] + 1;
  	int three = mesh_ptr->indices()[i+2] + 1;
  	int four = mesh_ptr->indices()[i+3] + 1;
...
bool Mesh::dumpPly(Mesh* mesh_ptr, std::ofstream& plyfile, std::string filename = "export"){
	...
	// Face section.
	//primitive enum from al_graphics 0 points, 1 lines, 2 line strip, 3 line loop, 4 triangles, 5 triangle strip, 6 tri fan, 7 quads, 8 quad strip, 9 polygon
	if(prim==0 || prim==1 || prim==2 || prim==3 || prim==5 || prim==6 || prim == 8 || prim == 9 ){
		std::cout << "primitive type not supported " << std::endl;
		return false;
	} else if (prim == 4){
		plyfile << "element face " << int(mesh_ptr->indices().size() / 3) << std::endl;

	} else if (prim == 7){
		plyfile << "element face " << int(mesh_ptr->indices().size() / 4) << std::endl;
	}
	...
	} else if (prim==7){ //GL_QUADS

		for (int i = 0; i < mesh_ptr->indices().size(); i+=4){
			int one = mesh_ptr->indices()[i];
			int two = mesh_ptr->indices()[i+1];
			int three = mesh_ptr->indices()[i+2];
			int four = mesh_ptr->indices()[i+3];

			plyfile << "4 " << one << " " << two << " " << three << " " << four << std::endl;
		}
	}
  return true;
}
